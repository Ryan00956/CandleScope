    ...
    *,
    ...
    if incremental_script:
        ...
        for seed_attempt in range(seed_attempts):
            ...
            cached_seed = (
                seed_query_cache.get(seed_cache_key)
                if isinstance(seed_query_cache, dict)
                else None
            )
            cache_age = (
                time.monotonic() - float(cached_seed.get("at", 0))
                if isinstance(cached_seed, dict)
                else float("inf")
            )
            seed_correction = str(
                (seed_revision or {}).get("correctionRevision", "0")
            )
            seed_closed = int(
                (seed_revision or {}).get("closedThrough") or 0
            )
            cached_closed = (
            ...
            seed_queried = not (
                ...
                == seed_correction
                and seed_closed <= cached_closed
            )
            seed_query = getattr(dm, "query_latest", None)
            seed_query_result = None
            attempt_seed_bars: list[Any] | None = None
            if seed_queried and callable(seed_query):
                seed_query_result = await run_storage(
                    seed_query,
                    symbol,
                    interval,
            ...
            elif not seed_queried:
                seed_query_result = cached_seed["result"]
            if seed_query_result is not None:
                attempt_seed_bars = confirmed_indicator_seed_bars(
                    list(seed_query_result.bars or [])
                )[-int(history_limit):]
            if range_service is not None:
                reset_once = getattr(
                    _pyne_incremental_sessions,
                ...
                if not callable(reset_once):
                    raise RuntimeError(
                        "Pyne seed revision guard requires reset support."
                    )
                reset_once(
                    str(meta.get("pyneSessionKey") or ""),
                    (
                        "seed-state",
                        _revision_token(seed_revision),
                        int((seed_revision or {}).get("closedThrough") or 0),
                    ),
                    lambda: PyneIncrementalSession(
                        script=meta["script"],
                        params=(
            ...
            initial_coverage = (
                initial.get("range") if isinstance(initial, dict) else None
            )
            initial_closed_end = (
                int(initial_coverage.get("end", 0))
                if isinstance(initial_coverage, dict)
                else 0
            )
            observed_closed = int(
            ...
            if (
                ...
                != _revision_token(seed_revision)
                or observed_closed > initial_closed_end
            ):
                seeded = False
                initial = None
                if seed_attempt + 1 >= seed_attempts:
                    raise RuntimeError(
                        "Pyne seed state changed repeatedly."
                    )
                continue
            seed_revision = observed_revision
            if (
                seed_queried
                and seed_query_result is not None
                and isinstance(seed_query_cache, dict)
            ):
                store_indicator_seed_cache(seed_query_cache, seed_cache_key, {
                    "at": time.monotonic(),
                    "result": seed_query_result,
                    "correctionRevision": str(
                        (observed_revision or {}).get(
                            "correctionRevision",
                            seed_correction,
                        )
                    ),
                    "closedThrough": initial_closed_end,
                })
            break
    ...
    async def _run_correction_queue() -> None:
        ...
        try:
            ...
            if deferred_realtime is not None and not meta.get("_disposed"):
                ...
                if (
                    meta.get("scriptMode") != "incremental"
                    or deferred_time > seed_end
                ):
                    task = asyncio.create_task(
    ...
    async def _run_realtime_event(event: Any) -> None:
        ...
        if (
            ...
            and msg.get("type") == "indicator.patch"
        ):
            msg["reason"] = "bar_closed"
        if range_service is not None:
