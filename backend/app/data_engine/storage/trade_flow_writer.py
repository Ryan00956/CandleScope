"""Bounded writer for one-minute trade-flow rollups."""
...
from .trade_flow_store import TradeFlowRollupStore
...
class TradeFlowRollupWriter:
    ...
    Evictions and failures remain visible through diagnostics.
    """

    def __init__(
        self,
        store: TradeFlowRollupStore,
        *,
    ...
    def start(self) -> None:
        if self._closing:
            raise RuntimeError("trade-flow rollup writer is closed")
        if self._task is not None and not self._task.done():
            return
        self._task = asyncio.create_task(
            self._run(),
            name="trade-flow-rollup-writer",
        )
    ...
    async def enqueue(
        ...
        Queue capacity still applies backpressure, but callers that own a hot
        reducer can continue once the request is accepted and observe the
        returned acknowledgement separately.  :meth:`write` remains the
        durable, wait-until-committed convenience API.
        """
