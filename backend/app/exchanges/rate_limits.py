...
class RateLimitReservation:
    """Exact quota ownership handed from an admitting caller to transport."""
    ...
    def record_response(self, **kwargs: Any) -> bool:
        if self.settled:
            return False
        recorded = self.manager.record_response(self.rule, **kwargs)
        status_code = kwargs.get("status_code")
        body_code = kwargs.get("body_code")
        if (
            bool(kwargs.get("response_unknown"))
            or bool(kwargs.get("response_complete", True))
...
class RateLimitManager:
    ...
    def record_response(
        ...
        *,
        ...
        if response_unknown:
            ...
            return False
        response_is_rate_limited = status_code in {418, 429} or body_code in {
            "-1003",
            "50011",
        }
    ...
    def circuit_snapshot(self) -> dict[str, Any]:
        return {
            key: circuit.snapshot() for key, circuit in sorted(self._circuits.items())
        }
...
def _binance_used_weight(headers: Mapping[str, str]) -> float | None:
    candidates = [
        value for key, value in headers.items() if key.startswith("x-mbx-used-weight")
    ]
    if not candidates:
        return None
    try:
...
class RateLimitPolicy:
    ...
    def delay_for(self, market_type: str = "spot") -> float:
        ...
        return max(
            0.0, float(value if value is not None else self.default_delay_seconds)
        )
    ...
    def retry_429_backoff_for(self, market_type: str = "spot") -> float:
        ...
        return max(
            0.0,
            float(
                value if value is not None else self.default_retry_429_backoff_seconds
            ),
        )
