...
def test_chart_templates_execute_deterministically(
    ...
    outputs = [provider.step(_frame(index, close)) for index, close in enumerate(closes, 1)]
...
def test_chart_pyne_rejects_unknown_code_with_line_diagnostic() -> None:
    with pytest.raises(StrategyProviderError) as caught:
        compile_chart_pyne('strategy("bad")\nimport os\nif close > 1\n  target_position(1)')
...
def test_chart_revision_reuses_compile_identity_and_changes_with_source(
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        first, second = list(pool.map(lambda _: service.create_strategy_revision(payload), range(2)))
    assert first["revision_id"] == second["revision_id"]
    assert {bool(first["reused"]), bool(second["reused"])} == {False, True}
    assert first["base_revision_id"] == CHART_PYNE_REVISION
    changed = service.create_strategy_revision({**payload, "source_text": SMA.replace("sma(close, 3)", "sma(close, 4)")})
...
def test_quick_run_fee_identity_fails_closed(tmp_path: Path) -> None:
    ...
    assert RunCreateRequest(**valid).model_dump()["taker_fee_bps"] == "4"
    first_smoke = service.smoke_strategy_revision(str(revision["revision_id"]), valid, now_ms=2)
    second_smoke = service.smoke_strategy_revision(str(revision["revision_id"]), valid, now_ms=5)
