...
async def test_compute_batch_rejects_shared_script_ohlcv_window_before_execution(
    ...
    class UnexpectedSidecar:
        async def execute(self, runtime_request, **_kwargs):
            execute_calls.append(runtime_request.language)
            raise AssertionError("script sidecar execution must not start")

    monkeypatch.setattr(
        indicators_api,
        "_resolve_indicator_runtime_service",
        lambda _request: UnexpectedSidecar(),
    )
    one_bar = _bars(1)[0]
    script_item = {
        "jobKey": "script-job",
...
def test_compute_batch_mixed_runtime_failure_is_isolated_and_ordered(
    ...
    class FailingSidecar:
        async def execute(self, runtime_request, **_kwargs):
            routed_languages.append(runtime_request.language)
            raise RuntimeError("mixed script exploded")

    monkeypatch.setattr(
        indicators_api,
        "_resolve_indicator_runtime_service",
        lambda _request: FailingSidecar(),
    )
    response = _client().post(
        "/api/v1/indicators/compute/batch",
        json=_body([
