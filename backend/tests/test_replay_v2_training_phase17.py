...
async def test_phase17_additive_v9_migration_backfills_deterministic_review_frame(
    tmp_path: Path,
) -> None:
    database = tmp_path / "phase17-migration.db"
    service = await _service(database)
    try:
        assert service.training is not None
        created = await service.training.create_run(await _request(service))
    ...
    phase17_tables = (
        "replay_review_fork_lineage",
        "replay_review_cursor",
        "replay_review_drawing_document",
        "replay_review_viewport_sample",
        "replay_review_event_anchor",
        "replay_review_timeline_event",
        "replay_review_marker",
    ...
    with sqlite3.connect(database) as connection:
        ...
        for table in phase17_tables:
            connection.execute(f"DROP TABLE {table}")
        connection.execute("PRAGMA foreign_keys = ON")

    restored = await _service(database)
    try:
        assert restored.training is not None
        rules = await restored.training.rules(run_id)
        assert rules["schema_version"] == "replay.run-rules.v1"
        assert [item["revision"] for item in rules["history"]] == [1, 1, 1]
        review = await restored.training.start_review(run_id, event_id=None)
        assert len(review["events"]) == 1
        assert review["events"][0]["event_type"] == "PHASE17_BACKFILL"
        assert review["immutability_proof"]["verified"] is True
        with sqlite3.connect(database) as connection:
