...
2026-08-05 immutable-T0 cutover：公开身份仍统一为 `run_id`。`POST /runs` 创建 `AWAITING_MARKET` 空 Run 时不读取 catalog、不绑定 symbol/dataset、不加载首图，但会原子写入 `replay.time-commitment.v1`：手动时间直接冻结，区间随机由服务端 seed 只抽取一次。用户进入 `?run=<id>` 后通过 Run 专属目录选择首个商品；目录只判断固定 `T0` 的上市、覆盖、周期和模式兼容性，不能 clamp、顺延或重抽。首个兼容商品再原子创建 MarketTrack 和可播放 cursor。schema 升至 v13 并要求清空开发数据，不提供旧 one-shot create 或旧 schema 迁移。
...
onNeedMoreLeft
  ...
  -> source SeriesWindowStore prepend delta
  -> ViewerState projection prepend (+ optional boundary mid-merge)
  -> shared chart adapter atomic commit + viewport anchor compensation
```

architecture test 必须禁止 replay provider 调用 live Kline API。
分页 pending 只进入状态栏，不得传给 `SingleChartPanes.loading` 触发整图遮罩。ViewerState 仅在初次装载、周期/身份切换或权威 stream resync 时发布 `replace`；普通历史页和播放推进必须沿用实时行情的 `prepend`、`mid-merge`、`tick`、`append` 语义。
...
- 此时不创建任何商品轨道，但 Run 自身的 `T0` 已冻结。用户在 Run 内显式选择兼容商品后才创建 `FULL` 主轨；快照内其他商品按需添加，未激活时保持 `NONE`；
...
- 提示不得写入 Run 的 `last_symbol`、MarketTrack、dataset 或 pin，也不得决定或修改全局 `T0`；缺失提示的直接 Hub Run 在首选品时生成 `DIRECT_HUB` 空快照；
...
Phase 7 已实现 `replay.data.segment.v1`：不可变 identity/checksum、prepare job、进度、取消、进程中断恢复、single-flight、quarantine、可信 rehydration、引用 pin、plan-hash GC 与 Windows 文件锁恢复均有测试。Phase 3 也已有 replay-only history endpoint 与前端 before-page provider；`DURATION` 只读取冻结 `replay_dataset_ref`，`ALL_AVAILABLE` 则由 replay service 按创建时绑定的连续边界读取本地只读 K 线仓库，不接入 live HTTP/WS、交易所下载或浏览器实时缓存。
...
2. `DURATION` 必须为 base interval 的正整数倍；`ALL_AVAILABLE` 定义为“所选起点所在、无 gap 的连续历史段起点”，不跨缺口伪造连续覆盖。`DURATION` 的有效 warmup 为 indicator 与 visible rows 的较大值；`ALL_AVAILABLE` 的执行 warmup 只取 indicator，左侧历史通过有界 before-page 按需读取，不占 `max_bar_dataset_rows` 执行快照预算。
...
4. selection 保持一个权威结果：先用请求 `catalog_epoch + server seed/manual start` 选定窗口，再按 effective warmup 构建执行快照；必须保持 source fingerprint 和已选起点，不能重新随机或接受漂移。adapter 继续只拥有一个有界冻结 snapshot。
5. segment manifest range 覆盖执行 snapshot（effective warmup start 到 forward end），并携带 data-policy role/count；`ALL_AVAILABLE` 的 `actual_visible_history_start_ms` 可早于 segment 起点。Run、dataset epoch、start-selection commitment 与领域 state hash 不因 history before-page 读取而变化。
6. history 升级为严格 `replay.history.v2`：返回 public history boundary 与 canonical policy，只显示边界之后的 closed bars；`DURATION` 从冻结 snapshot 读取，`ALL_AVAILABLE` 从 replay service 的本地只读仓库按固定边界、最多 100,000 基础行分段读取并逐行校验。盲化 Run 的查询和响应都映射到 synthetic timeline。`NONE` 在加载 dataset 前拒绝，WARM 可准备/读取但不推进，FULL 才进入全局时钟。
   前端沿用有界 `SeriesWindowStore`：深度左翻淘汰右端后，右侧触边必须走 replay stream 的原子 resync 恢复最新已揭示窗口；恢复成功后清空旧 history epoch 并允许重新左翻，虚拟时钟、账户和 data epoch 均不前进。
...
8. BAR 与 AGG 共用 chart-only K 线左侧历史；AGG 的成交源仍只覆盖 forward replay 窗口，不把历史 K 伪装成旧成交/order-flow。左侧分页不得触发下载；缺行、错序、身份漂移或 gap 均 fail closed。

### 测试与退出门槛

- manual/random × BAR/AGG × DURATION/ALL_AVAILABLE 的 selection、effective rows、gap、预算、catalog/source drift 均有 reference matrix；特别证明 ALL_AVAILABLE 的可见 rows 可大于执行 warmup 和执行快照预算，同 seed 与同输入重启后边界和 policy hash 不漂移。
...
- provider 中断、schema/identity/range/gap 错误、retry、并发 single-flight、取消与重启恢复复用 Phase 7 真实文件/SQLite 测试，并增加 canonical data-policy/manifest 集成证据。
...
16. 绑定 `cd407196c63c40a326bc40ea70d0da687f2a51e2` 的正式真实 BAR soak 已越过 cycle 24 的旧暂停故障，但在约 76 分钟、完成 32 轮后，cycle 33 的 return-to-Hub 收到 `409 TRAINING_RUN_BUSY`，内部 reason 为 `SESSION_NOT_FOUND`；失败 artifact 正常落盘并完整清理后端、Vite、Chrome 与端口。诊断同时显示 lifecycle/lease pending 均为 0、reaper/recovery/transaction failure 均为 0，而累计已有 128 次 lazy recovery 与 161 次 eviction，因此不是数据损坏或持久化失败。根因是 `release_session_to_hub` 先用 `get_session()` 恢复并释放瞬时租约，再获取 `_prune_lock` 声明 Hub eviction；idle reaper 可在两者之间把刚恢复的 handle 驱逐，形成确定的 TOCTOU 窗口。修复改为直接取得 handle lease，并在 `_prune_lock` + `_lifecycle_lock` 内无 await 地把调用方唯一 lease 原子转移为 `evicting` claim；若存在额外 in-flight lease 仍严格返回 busy，恢复、容量与 reaper 的既有锁序不变。新增对抗回归在 lease 释放瞬间推进假时钟并启动真实 prune：旧两段式路径会先驱逐再返回，新路径则让 prune 等到 Hub eviction 完成；该用例连续 `50/50`，Phase 1 + service `23 passed`，训练 API/归档关联 `2 passed`，Ruff、compileall 与 whitespace PASS。首轮后端全量在高负载下于既有 shutdown 测试的 1 秒外层截止得到 `1 failed, 2,106 passed`，该用例独立连续 `30/30` 未复现，原样第二轮全量为 `2,107 passed`、4 条既有弃用警告、146.80 秒；未修改产品 shutdown 或放宽任何门禁。此修复独立提交后，`cd407196` 的 checks、真实来源、smoke、benchmark 与失败 soak 全部只作排障证据，正式发布 artifact 必须重新绑定新 clean HEAD 并从零生成。
17. `44256a09ddb2041ee34dfbf5dc40443f427093b7` 的首次 clean-HEAD checks 在 backend pytest 阶段超过 10 分钟且无 artifact；只读进程环境确认 `PYTEST_CURRENT_TEST` 停在既有 `test_full_order_book_ws_applies_requested_grouping_from_cached_tick (call)`，主线程与 28 个工作线程均处于等待，随后按精确根 PID 清理四级孤立进程树。根因不是 full-order-book 产品停滞，而是测试在初始 snapshot 后立即发 unsubscribe，同时仍无界等待“排队 stale status + unsubscribe ack”两个消息；服务端以 read/forward 任一任务先完成为连接终止条件，高负载下 unsubscribe 合法先胜出并取消尚未发送的 queued status，第二个 `receive_json()` 因此永不返回。修复只调整测试时序：先消费 fixture 明确发布的 stale status，再发 unsubscribe 并只等待其 ack；不改变 WebSocket 协议、服务端 FIRST_COMPLETED 或队列排空语义。修正用例连续 `50/50`，full-order-book 文件 `5 passed`，Ruff PASS；随后后端全量 `2,107 passed`、4 条既有弃用警告、144.77 秒。该门禁稳定化形成新 HEAD 后，`44256a09` 的空证据目录不构成 artifact，所有正式 checks 与后续发布证据仍从新 clean HEAD 起算。
18. 绑定 `f142bf1c7406b76c8b33f0408cfb0f48c908f32c` 的正式真实 BAR soak 已越过旧 cycle 33 竞争并运行至约 144 分钟，但在 cycle 58 的新 lifecycle target 以 `Timed out waiting for lifecycle recovery snapshot` 失败。失败现场的目标 URL/title 正确、HTML 与 Vite HMR WebSocket 已到达，但 `readyState=interactive`、`#root` 为空且没有任何 replay API 请求；后端 Run/Session/actor 均为健康 PAUSED，subscriber=1，reaper/recovery/transaction/shutdown failure 全为 0，lease/lifecycle pending 全为 0。根因是正式 soak 每轮都对 Vite 开发服务器创建新 target，并配合 `Network.setCacheDisabled(true)` 与 `Page.reload(ignoreCache=true)` 重新请求大型源码模块图；第 58 轮入口模块未完成，属于开发态基础设施耗尽而非产品恢复失败。harness 改为清除宿主任意 `VITE_*`、固定 `NODE_ENV=production`，再用当前 release flags 构建 OS 临时目录内的独立 production dist，并由同一 Vite 配置的 `preview` 代理 HTTP/WebSocket；构建失败、超时或 manifest 漂移在启动后端/Chrome 前即写 fail-closed artifact。百万事件投影不再依赖开发态 `/src/...` URL：只在 `VITE_REPLAY_SOAK_PROJECTION_ENABLED=1` 时增加一个 production Rollup entry，由 `.vite/manifest.json` 精确解析唯一且真实存在的哈希 JS；普通 production build 不包含该入口。artifact 新增显式构建环境、命令 hash/时长、每个输出文件 bytes/SHA-256、整体 manifest hash 与投影资源绑定，临时绝对路径不进入成功证据。新增 plan、端口/临时绝对目录、环境清洗、manifest 唯一入口/路径逃逸/缺失资源和逐文件证据回归，脚本套件 `24 passed`；TypeScript、ESLint、Node syntax 与 whitespace PASS。真实双构建验证普通 manifest 的 projection entry 为 0、soak manifest 为 1；完整 `npm run check` 的 architecture、双 TypeScript、ESLint、`2,468 tests` 与普通 Vite production build 全部通过。该修复提交后必须先在 clean HEAD 上通过 production-preview 短程 lifecycle 压力，再从零重建 checks、真实来源、v1/v2 smoke、formal benchmark、4h soak、rollback 和最终 manifest；`f142bf1c` 的所有 PASS/失败 artifact 均不得继承。
19. `34fbb50fd57dcb8d95cb407241be554acc5cedfc` 的 clean-HEAD production-preview 60-cycle 压力在训练循环前的 10,000-event 浏览器投影立即以 `ReplayStore is not a constructor` 失败；artifact 证明 production build、preview、后端和 Chrome 均正常启动并在失败后完整清理。真实哈希入口只导出 Rollup 内部共享符号 `R/r`：Vite 对无外部消费者的应用入口默认不保留声明的 ESM 入口签名，manifest 能证明文件身份却不能证明导出合同。修复只在 `VITE_REPLAY_SOAK_PROJECTION_ENABLED=1` 的测试构建设置 Rollup `preserveEntrySignatures=strict`，普通 production 构建配置不变；构建检查进一步解析最终生成的 ESM export block，必须同时存在 `ReplayStore`、`fixtures` 和 `parser`，否则在启动服务前 fail closed。真实 production build 的哈希入口经 Node 动态导入返回精确三项 key，`ReplayStore` 为 function，parser/fixture 各含两项预期函数；脚本 `24 passed`，TypeScript、ESLint、Node syntax 和 whitespace 通过。第一次完整前端检查在 `2,467/2,468` 处出现一次未在相同代码复现的单例失败；原样全量测试随即 `2,468/2,468`，下一次完整 `npm run check` 的 architecture、双 TypeScript、ESLint、`2,468 tests` 和普通 production build 全部 PASS。既有并发 Vite middleware 的 24678 HMR 端口提示仍只是仓库已记录的测试噪声，未修改或放宽门禁。此修复必须形成新 clean HEAD 后重新执行 60-cycle 压力；`34fbb50` 的失败 artifact 只作诊断，不能成为正式发布证据。
20. 绑定 `c988fb23bd5d0a5f9d95213b4bed8b46d0e0f451` 的正式 production-preview soak 已完成百万事件投影并连续运行 `13,995,321 ms`，在第 91 个训练动作等待 MARKET order ACK 时 fail closed。失败 artifact 记录两次命令均为 503 `PERSISTENCE_DEGRADED`、2 次 persistence transaction failure 和 1 次 reaper shutdown failure；同一 Run 的 review budget 已到 `536,839,811 / 536,870,912 B`，只余约 31 KiB。根因不是订单确认、WebSocket 或 SQLite 随机竞态，而是每个关键事件都把随订单/fill/ledger 历史增长的完整 canonical actor checkpoint 原样复制为不可变 anchor，使物理占用随时间近似二次增长；下一次命令和 reaper checkpoint 同时无法再写。修复不删除 Review/Fork 证据、不提高 512 MiB 上限，也不改变 actor checkpoint/state hash/event-chain：additive v9 表增加 `payload_encoding` 与 `stored_bytes`，新 anchor 使用确定性 `ZLIB_V1` 或小 payload `RAW`，原始长度和 SHA-256 继续独立保存；读取在声明原始长度内有界解压，并复核 stored length、原始 length、EOF/尾随字节与原始 SHA-256，损坏统一 `REVIEW_ANCHOR_CORRUPT` fail closed。预算、Hub inventory 与存储治理按实际 BLOB 字节统一计量；旧 RAW 行自动补齐 stored length，禁用/default-off rollback 二进制按列默认值新写的 RAW 行可在再升级时恢复。codec、损坏/压缩炸弹边界、40-anchor 压缩、compressed exact Fork、legacy RAW 重启迁移与 exact Fork 回归通过；Phase 17 + 18 扩大集合为 `22 passed`。该修复形成新 clean HEAD 后必须先通过 100-cycle production-preview 容量诊断，再从零重建全部正式 artifact；`c988fb23` 的 checks/benchmark PASS 和失败 soak 均只作诊断，不能进入最终 manifest。
21. 锚点 codec 修复的首轮后端全量准确捕获训练包显式白名单缺少 `anchor_codec.py`；将新生产模块明确纳入契约后，package contract `27/27` 通过，最终后端全量为 `2109 passed`（4 条既有 FastAPI 警告），前端 `npm run check` 为 `2468 passed`，TypeScript、ESLint、架构门禁与常规 Vite build 全部通过。该结果验证实现工作树，但仍不替代新 clean HEAD 的正式 release artifact。
22. 锚点 codec 的 clean-HEAD 100-cycle 诊断先由系统 Chrome/Edge 中 Kaspersky 注入的外部 HTTP/WebSocket 被 no-external-network 门禁如实拒绝；门禁未放宽，改用固定版本并核验 SHA-256 的官方 Chrome for Testing headless shell 后，100 轮生命周期与压缩锚点均完成，但最终报告页面停在“正在恢复回放”。失败诊断显示 durable 写入无失败、actor 已全部回收，而同一 session 同时存在 2 个 pending handle acquisition/lifecycle owner。根因是 lazy recovery owner 在 `_recover_record` 内先把新 handle 发布进 `_sessions`，随后才回到 `_acquire_handle_lease` 激活 lease；1 秒 idle reaper 可在这段 publish-before-lease 窗口立即驱逐刚恢复的 handle，触发恢复/回收抖动并饿死最终报告请求。修复让 prune 候选显式排除仍在 `_pending_recoveries` 中的 session，不延长 TTL、不放宽容量、不改变恢复 singleflight 或取消语义；新增确定性回归把 owner 阻塞在发布后、lease 前并并发运行真实 prune，证明 recovery claim 在 lease 激活前保护 handle。lazy recovery 定向 `4 passed`，service + shutdown 扩大集合 `33 passed`，Ruff、compileall 与 whitespace PASS。该修复形成新 clean HEAD 后仍须先重跑 100-cycle 诊断，再从零生成全部正式 artifact。
23. `82b31d9ee072381da46546713ff55e6e11298549` 的 clean-HEAD 100-cycle 诊断再次完成 10,000-event 浏览器投影、100 轮生命周期与 101 个 Run，875 个 actor anchor 全部为 `ZLIB_V1`、实际约 25.5 MiB，持久化 transaction/busy/recovery/reaper failure 均为 0，但最终报告仍在 120 秒硬截止失败。新保护使失败瞬间明确保留 recovery claim，进一步对齐最近 30 个 HTTP request/response 后确认多个 v1/v2 report 已先后 200 成功；真正的剩余 liveness 缺口是 `ENDED` actor 在报告页分阶段 HTTP/WS 初始化的零租约间隙被后台 reaper 无条件立即回收，导致页面不断“成功恢复→再次回收→再次恢复”，而非单次恢复永久卡死。修复把回收意图分开：容量预留/恢复前的 pressure prune 仍可立即回收 ENDED actor，周期后台 reaper 则尊重最近活动后的固定 5 秒 HTTP-to-WebSocket handoff grace；宽限不延长普通 idle TTL，也不改变最大会话数。新增回归先以容量压力立即回收 ENDED actor，再从 durable report 恢复，证明后台 prune 不会打断第二个 report，5,001 ms 后仍准确回收。ended/lazy 定向 `7 passed`，service + shutdown `34 passed`，Phase 1 + API `16 passed`（仅 4 条既有 FastAPI 弃用警告），Ruff、compileall 与 whitespace PASS。该修复形成新 clean HEAD 后仍须重跑 100-cycle，旧 `82b31d9` 失败 artifact 只作排障证据。
24. `b975d03d943df9c8da819a1c423cd685cc255246` 的 clean-HEAD 诊断已越过此前最终报告 liveness 路径，但在 cycle 76 的显式 primary adapter Hub eviction 断言 fail closed：目标 release 第一次即成功，Hub eviction 已累计到 151，而总 eviction 在同一观测窗从 226 增至 228。新增的 5 秒后台宽限使另一 ENDED actor 恰在该窗口自然到期，暴露 harness 把“总回收必须恰好 +1”误当成目标 Hub 回收证据；这不是产品丢失目标 eviction。fixture 现同时记录 Hub before/after，harness 严格要求目标 session 相同、release attempt 有界、Hub counter 精确 +1、总 counter 至少 +1，从而允许可解释的并发后台回收但不能掩盖错误目标或漏记 Hub 回收。新增纯函数回归覆盖 total +2 / Hub +1 的真实现场，并拒绝 Hub +2、total 无增长和错误 session；replay-soak 脚本 `25 passed`、Node syntax PASS，smoke fixture `11 passed`、Ruff PASS。该 harness 修复形成新 clean HEAD 后仍须重新执行完整 100-cycle；`b975d03` 的 cycle-76 失败 artifact 不得进入正式清单。
...
Commands run:
  ...
  clean-HEAD v2 formal stability: node scripts/replay-soak.mjs --duration-ms 3600000 --cycles 100 --projection-events 1000000 --sample-ms 60000 --out <external HEAD>/replay-v2/replay-v2-soak.json
...
Browser/accessibility evidence: 2026-08-10 owner 决策后，正式稳定性 artifact 必须为 mode=release-stability-60m、duration>=3,600,000 ms、cycles>=100、projectionEvents>=1,000,000；每个 archive cycle 必须 return-to-Hub checkpoint/release、PAUSED 恢复、ENDED、report、read-only review 且原 state hash 不变。Hub 创建、下单、Space play/pause、ArrowRight step 使用真实键盘事件；结束确认验证对话框描述、焦点环、Escape 恢复；CDP emulation 验证 prefers-reduced-motion 生效。短 harness 不能替代正式稳定性门禁；4 小时 artifact 仅允许 mode=observation-4h-non-blocking，不能进入 release PASS。
...
Evidence binding: 所有发布 artifact 必须位于仓库外含完整 git_head 的目录；每份 JSON 自带 release_evidence.git_head/git_dirty=false。checks、smoke 与稳定性 artifact 必须属于当前 HEAD。benchmark、真实来源与 rollback 可从祖先 HEAD 复用，但 verifier 必须证明声明的阶段运行时输入 pathspec 零 diff，并在 release-manifest 写入 captured/current HEAD、完整 changed-files 与 artifact SHA-256。release-manifest 仍执行提交回滚并再次确认 clean HEAD；它是 PASS 的唯一权威。
