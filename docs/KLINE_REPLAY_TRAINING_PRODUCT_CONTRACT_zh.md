...
状态：`FROZEN_PHASE_0`。用户已于 2026-07-21 确认本合同；Phase 0 冻结协议、开关与架构护栏，但所有 v2 产品入口和运行时仍默认关闭，尚不表示 Phase 1–10 功能已经实现或启用。
...
1. 用户从普通实时行情页顶部进入回放；回放在独立的 replay document 中运行，不在 live 组合根里切换数据源。
2. 回放页首先展示“训练存档大厅”页面内弹窗，可新建训练，也可恢复已有训练。
...
```mermaid
flowchart LR
    A["实时行情页"] -->|顶部 K 线回放入口| B["独立 replay document"]
    B --> C["训练存档大厅"]
    C -->|新建训练| D["配置与能力校验"]
    C -->|加载存档| E["恢复并校验数据引用"]
    D --> F["创建 TrainingRun"]
...
- 点击后打开独立 replay document；live 页面保持原商品、周期、socket、缓存和布局状态。
- 入口需要同时满足前端展示开关和后端权威 capability；后端不可用时不得打开一个会回退到 live 的假回放页。
- 入口将当前实时页的商品身份作为“新建训练默认商品”传给 replay document，但不得携带隐藏的历史起点、未来数据或 live runtime 对象。
- 直接访问 replay URL 也必须能进入存档大厅，不依赖 `window.opener`。

### 4.2 训练存档大厅

存档大厅是 replay document 内的模态工作区，不是只能新建 Session 的一次性表单。
...
1. 从 live 的 BTCUSDT/15m 打开 replay，live 页面不漂移，存档大厅默认 BTCUSDT。
