...
Run 创建本身必须原子提交，因此不定义 `CREATING` 半状态。`AWAITING_MARKET` 不是半成品：它是商品无关、可列出、可删除、可继续选品的完整 Run。首个商品初始化另有独立原子事务；初始化失败时 Run 必须保持 `AWAITING_MARKET`。
...
- 入口可以把当前商品身份作为首选搜索提示，但不得把它写成 Run 身份或创建前置条件；不得携带隐藏的历史起点、未来数据或 live runtime 对象。
- 用户创建或加载训练后才打开独立 replay document；公开 URL 只使用 `?run=<run_id>` 恢复上下文，不依赖 `window.opener`，adapter session id 不得成为用户可见身份。
...
| 商品 | 不属于创建字段。进入空 Run 后搜索并选择首个商品；训练中可继续从 Run 目录添加并切换其他商品。最近查看商品只是 ViewerState/存档摘要。 |
...
能力校验与 MarketTrack 初始化必须绑定同一个版本化 catalog/data epoch。校验后数据发生变化时，选品要拒绝并要求刷新，不能使用过期结论；空 Run 本身不依赖 catalog epoch。
...
19. 已初始化 Run 可从目录搜索并添加任意同账户范围商品；地址和 `run_id` 不变，切到新商品时只加载该商品，`NONE` 商品保持零历史读取。
