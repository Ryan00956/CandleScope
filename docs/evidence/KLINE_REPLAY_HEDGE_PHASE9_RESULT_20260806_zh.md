...
- background capacity 与 preview 现在统一使用 500 ms trailing scheduler，游标持续变化或用户快速提交时只保留/取消尚未启动的回调；开始 exact 下单校验时会取消 scheduler、终止在途 advisory，并在校验期间禁止重新调度。exact capacity/preview 共用一个 15 秒 AbortSignal，重复提交被禁用，卸载时所有 timer/controller 都释放。soak 同时观察页面错误提示，校验失败后立即返回具体原因，不再固定等待 120 秒。
...
- `d5141bbe7f94d167126867495373484ea134f7dd` 的第二轮 10-cycle 仍完成全部产品动作并通过 API/命令身份合同；advisory 降至 162（92 capacity、70 preview），其中 128 个不同正文、34 个跨 cursor/lifecycle cancel 后的重复正文，但仍高于 140，故继续保持 FAIL。动作流程在 PAUSED 后只等待 250 ms 就进入 set-speed/exact-order；180 ms timer 会在这段确定会被下一动作取消的窗口内启动后台请求。最终 debounce 固定为 500 ms：真实用户停留半秒仍得到参考预览，自动化或快速操作直接走点击时的 exact 双校验，不再先制造必然取消的后台请求；预算仍为 140，没有随测量结果上调。
...
- 最终 500 ms debounce 的 10-cycle 分钟级高压回归以 `97/140` advisory（70 capacity、27 preview）通过，10/10 训练与归档生命周期、10,000 projection events、heap、blind audit 和全部产品动作均完成。增量验证为 keyed scheduler `3 passed`、soak harness `63 passed`、replay 前端 `351 passed`、Phase 10 发布合同 `12 passed`，architecture、双 TypeScript、定向 ESLint、Node syntax 与 `git diff --check` 全部通过。后续只重跑当前 HEAD 必需的 checks、smoke、rollback 和一次 60 分钟门禁，benchmark/真实来源仅按祖先与阶段 pathspec 零 diff审计复用，旧 4 小时观察不重跑。

### 3.34 60 分钟门禁捕获暂停确认内的穷举账户审计

- advisory 修复后的 clean HEAD `5baa80d979ffcc26841e319441d6a4add39d1288` 没有重跑旧 4 小时观察；全量 checks 为 backend `3,256 passed`、frontend `3,003 passed`，短 smoke 与 rollback 均 PASS。唯一一次正式 60 分钟运行完成百万事件投影（`89,412.65 events/s`）并运行约 `3,857 s`，在第 73 个训练周期、74 笔订单/74 笔成交后等待 post-fill PAUSE 确认超时。页面、Edge、FastAPI、SQLite 和 WebSocket 仍存活且无 console/runtime/persistence failure；PAUSE POST 已发出但无响应，同时无需 Run actor 锁的 integrity/drawings 查询仍返回、需要该锁的 tracks/rules/equity/capacity 查询全部等待，因此不是浏览器被清理软件关闭。
- 根因是 server-owned ordered playback 虽已把有持仓批次限制为每次一根 BAR，却仍在持有同一把 Run actor 锁时，于每根 BAR 完成或 PAUSE committed barrier 执行 `audit_account`。该审计会独立重建完整 HEDGE 账户、成交和哈希链；账本随训练周期增长后，PAUSE 即使已提前设置 stop event，也必须等待穷举审计。前端的 30 秒命令确认超时随后进入 fail-closed tracks refresh，而该 refresh 又等待同一把锁，使控制栏继续显示“正在暂停”直至 harness 的 120 秒诊断截止。
- ordered playback 现在只在已事务提交的 market barrier 上让出锁，不在交互式逐 BAR 路径执行穷举账户审计；手动/有界 advance 的默认审计合同保持不变，最终结果与显式账户审计也不放宽。前端命令失败仍同步清空本地轨道投影，但不再等待同锁的权威 refresh 才释放 pending 状态；refresh 在服务端恢复后只用权威响应重建投影。
- 新增的压缩回归直接生成与失败现场相同的 74 笔订单/74 笔成交，并把 `audit_account` 替换为禁止调用的哨兵；真实 ordered BAR、stop barrier 与 PAUSE 必须在 1 秒内完成。该回归约 14 秒通过，旧实现会在进入穷举审计时立即失败；相邻 high-rate pause barrier `2 passed`，前端相关 `40 passed`、双 TypeScript 与定向 ESLint 均通过。产品根因通过后才允许对新 clean HEAD 再执行一次正式 60 分钟门禁，不以反复长跑代替定位。
