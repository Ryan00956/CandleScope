...
test("ordinal carrier keeps stable isolated keys when its source horizon advances", () => {
  ...
  assert.deepEqual(
    second.data.map((point) => point.time.order),
    first.data.map((point) => point.time.order),
  );
  assert.deepEqual(second.data.map((point) => point.time.sourceTime), [280, 340]);
  assert.ok(second.data.every((point) => point.time.order > 9));
});
...
test("the largest ordinal carrier allocation stays inside safe-integer space", () => {
  ...
  const lastOrder = plan.data.at(-1).time.order;
