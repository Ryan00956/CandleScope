...
function configureTimeFormatting(behavior, overrides = {}) {
  behavior.setOptions({
    localization: {
...
function axisItem(order, sourceTime, sourceOrdinal = 0) {
...
test("preprocessing and conversion validate the ordinal item contract", () => {
  ...
  assert.equal(converted._ordinal_sourceOrdinal, 1);
  assert.equal(converted._ordinal_sourceTime, 100);

  assert.throws(
    () => behavior.key({ order: "1", sourceTime: 100, sourceOrdinal: 0 }),
  ...
  assert.throws(
    () => behavior.cacheKey(axisItem(1, 100, 0)),
...
test("labels are formatted from sourceTime rather than ordinal order", () => {
  ...
  assert.equal(behavior.formatTickmark({
    index: 0,
    originalTime: axisItem(999_999, sourceTime, 3),
    time: internal,
    weight: 20,
  }, { locale: "en-US" }), `source:${sourceTime}`);
...
test("tick weights follow source-time boundaries while repeated timestamps stay ordinal", () => {
  ...
  assert.equal(behavior.maxTickMarkWeight(points.map((point) => ({
    coord: 0,
    label: "",
