...
import {
  ...
  renderMainSeriesProjectionPatch as renderProductionProjectionPatch,
...
import { partialMock, structuralMock } from "../../test/testHelpers.js";

type RenderOptions = NonNullable<Parameters<typeof renderProductionProjectionPatch>[0]>;
type BuildPatchOptions = NonNullable<Parameters<typeof buildMainSeriesProjectionPatch>[0]>;
type TestRenderOptions = Omit<RenderOptions, "series"> & {
  series?: Partial<ProjectionSeriesWriter<ChartSeriesInputRow>> | null;
};
...
function numericAnchor(value: unknown): number {
  if (!value || typeof value !== "object" || !("time" in value)) {
    throw new Error("Expected a numeric viewport anchor");
  }
  const time = value.time;
  if (typeof time !== "number") throw new Error("Expected a numeric viewport time");
  return time;
}

function indexForTime(value: unknown, indexes: Record<number, number>): number {
  return typeof value === "number" ? indexes[value] ?? -1 : -1;
}
...
test("projection renderer incrementally replaces the last point and appends", () => {
  ...
  const series: Partial<ProjectionSeriesWriter<ChartSeriesInputRow>> = {
...
test("projection renderer preserves viewport through setData instead of popping", () => {
  ...
  const result = renderMainSeriesProjectionPatch({
    resolveDisplayAnchorIndex: (time) => indexForTime(time, { 10: 0, 20: 1, 30: 2 }),
    ...
    viewportController: {
      ...
      applyAnchorShift: (anchor, resolver) => {
        viewportEvents.push(["apply", resolver(numericAnchor(anchor))]);
      },
    },
  });
...
test("projection renderer rebuilds structural tails", () => {
  ...
  const result = renderMainSeriesProjectionPatch({
    resolveDisplayAnchorIndex: (time) => indexForTime(time, { 5: 0, 10: 1, 20: 2 }),
    ...
    viewportController: {
      ...
      applyAnchorShift: (anchor, resolver) => {
        viewportEvents.push(["apply", anchor, resolver(numericAnchor(anchor))]);
      },
    },
  });
...
test("successful pop-update commits the rendered cache in place", () => {
  ...
  const patch = buildMainSeriesProjectionPatch({
    displayRows,
    previousSeriesData: structuralMock<ChartSeriesInputRow[]>(previousSeriesData),
...
test("a frozen rendered cache safely falls back to a new committed array", () => {
  ...
  const patch = buildMainSeriesProjectionPatch({
    ...
    previousSeriesData: structuralMock<NonNullable<BuildPatchOptions["previousSeriesData"]>>(
      previousSeriesData,
    ),
    projectionPatch: { fromOutputIndex: 2 },
    renderOptions: { chartType: "line" },
...
test("repeated lazy tail commits stay equivalent to fresh full conversion", () => {
  let displayRows: ChartSeriesInputRow[] = [];
  let renderedData: ChartSeriesInputRow[] = [];
  let nextTime = 1;
