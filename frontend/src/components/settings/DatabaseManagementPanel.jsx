...
function displayMarketType(value) {
...
function formatDateTimePrecise(ms) {
    if (!ms) return '--';
    return new Intl.DateTimeFormat('zh-CN', {
        year: 'numeric',
        month: '2-digit',
...
function intervalSortValue(interval) {
...
export default function DatabaseManagementPanel({
    ...
    const [backfillDialog, setBackfillDialog] = useState(null);
    ...
    const loadSeries = useCallback(async () => {
        ...
        try {
            ...
            setError(`数据库库存加载失败: ${err.message}`);
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        fetchDatabaseSeries()
            ...
            .catch(err => {
                if (!disposed) setError(`数据库库存加载失败: ${err.message}`);
            })
            .finally(() => {
    ...
    const removeSeriesFromView = useCallback((series) => {
        setPayload(prev => {
            if (!prev) return prev;
            return {
                ...prev,
    ...
    const removeSymbolFromView = useCallback((group) => {
        setPayload(prev => {
            if (!prev) return prev;
            return {
                ...prev,
    ...
    const handleScanGaps = useCallback(async (series) => {
        ...
        try {
            ...
            setOperationResult({ status: 'error', message: `扫描失败: ${err.message}` });
        } finally {
            setOperationLoadingKey('');
        }
    }, []);

    const openBackfillDialog = useCallback((series) => {
    ...
    const updateBackfillDialog = useCallback((patch) => {
    ...
    const submitBackfill = useCallback(async () => {
        ...
        try {
            ...
            setOperationResult({ status: 'error', message: `补全失败: ${err.message}` });
        } finally {
            setOperationLoadingKey('');
        }
    }, [backfillDialog]);

    const openDeleteSeriesDialog = useCallback((series) => {
        setDeleteDialog({ type: 'series', series, confirmText: '' });
    }, []);

    const openDeleteSymbolDialog = useCallback((group) => {
        setDeleteDialog({ type: 'symbol', group, confirmText: '' });
    }, []);

    const updateDeleteConfirmText = useCallback((value) => {
    ...
    const submitDelete = useCallback(async () => {
        ...
        if (deleteDialog.type === 'series') {
            ...
            try {
                ...
                setOperationResult({ status: 'error', message: `删除失败: ${err.message}` });
            } finally {
                setOperationLoadingKey('');
            }
            return;
