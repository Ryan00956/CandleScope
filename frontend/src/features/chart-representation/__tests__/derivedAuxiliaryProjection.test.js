...
test("display source index keeps display order, exact source groups, and time identity", () => {
  ...
  assert.strictEqual(index.bySourceTime.get(20)[0], rows[2].time);
...
test("timed entries fan out in display order with exact matches and last-write-wins", () => {
  ...
  assert.throws(
    () => projectSourceTimedEntries(entries, index, { fanout: "nearest" }),
...
test("barcolor groups fan out to every exact display target and drop missing targets", () => {
  ...
  assert.deepEqual(projected[0].data.map((point) => point.color), ["red", "red", "green"]);
  assert.deepEqual(projected[0].data.map((point) => point.time), [
    rows[0].time,
    rows[1].time,
    rows[3].time,
  ]);
...
test("last display target lookup compares only the current and next exact source time", () => {
  ...
  assert.equal(isLastDisplayTargetForSourceTime([{ time: "10" }], 0), false);
