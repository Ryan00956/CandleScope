...
function row(time, close, isClosed = undefined) {
  const result = {
    time,
    open: close,
    high: close,
...
function outputOrder(item) {
  return item?.time?.order;
}

function overlayProjection(confirmed, provisional) {
...
function referenceProjection(projector, rows, { seedState = null } = {}) {
  ...
  const trial = projector.projectWithState([rows.at(-1)], {
    provisional: true,
...
function provisionalFlags(rows) {
  return rows.map((item) => item.customValues?.chartProjection?.provisional);
}

function assertMatchesReference(store, descriptor, rows, options = {}) {
