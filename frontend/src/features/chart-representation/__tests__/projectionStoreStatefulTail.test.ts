...
interface MutableCountingStatefulProjector extends StatefulProjector {
  projectedRows: number;
}

interface CallingStatefulProjector extends StatefulProjector {
  calls: number;
}
...
interface ProjectorCase {
  close: number;
  expected: PatchExpectation;
  name: string;
}

interface StatefulProjectorDescriptor {
  name: string;
  create(): StatefulProjector;
  appendBase: number[];
  appendCases: ProjectorCase[];
  replaceCases: Array<ProjectorCase & { initial: number[] }>;
}

function row(time: number, close: number): SourceBar {
  return { time, open: close, high: close, low: close, close, volume: time * 10 };
}

function source(closes: number[]): SourceBar[] {
  return closes.map((close, index) => row(index + 1, close));
}

function outputOrders(rows: readonly DisplayRow[]): number[] {
  return rows.map((item) => {
    if (!isOrdinalAxisTime(item.time)) throw new Error("Expected ordinal projection output");
...
class CountingStatefulProjector implements Projector {
  readonly projector: StatefulProjector;
  readonly id: string;
  readonly oneToOne: boolean;
  supportsStatefulTailProjection: boolean;
  projectedRows: number;
...
const PROJECTORS: StatefulProjectorDescriptor[] = [
  {
    name: "Renko",
...
function assertDisplayIndexIntegrity(store: ProjectionStore): void {
  ...
  [...timeSet].forEach((time, index) => assert.strictEqual(time, mustBeDefined(display[index]).time));
  for (let index = 0; index < display.length; index += 1) {
    const item = mustBeDefined(display[index]);
    assert.equal(store.indexOfDisplayTime(item.time), index);
    assert.equal(
      store.indexOfDisplayTime(isOrdinalAxisTime(item.time) ? { ...item.time } : item.time),
      index,
    );
    assert.strictEqual(store.getDisplayByTime(item.time), item);
  }
}
...
test("stateful tail updates keep an independent source array and preserve prior snapshots", () => {
  ...
  assert.equal(mustBeDefined(store.sourceSnapshot().at(-1)).close, 15);
...
test("stateful display changes stay copy-on-write while semantic no-ops retain identity", () => {
  ...
  const lineageIndex = mustBeDefined(store.drawingLineageIndex());
  ...
  const noOpRows = [
    ...appended.slice(0, -1),
    { ...mustBeDefined(appended.at(-1)), volume: 9999 },
  ];
...
test("drawing lineage snapshots recover after clearing an ordinal display", () => {
  ...
  const restoredOrdinalIndex = mustBeDefined(restoredSnapshot.ordinalSeriesIndex);
  assert.strictEqual(
    restoredOrdinalIndex.seriesData,
    restoredSnapshot.seriesData,
  );
  assert.equal(restoredOrdinalIndex.latestLineage, 11);
...
test("stateful tail patches update the private time index and lazily version the public time set", () => {
  ...
  const oldLastTime = mustBeDefined(oldDisplay.at(-1)).time;
...
test("a malformed projector with duplicate display orders never enters the incremental index path", () => {
  const projector: MutableCountingStatefulProjector = {
    id: "duplicate-orders-test",
    oneToOne: false,
    projectedRows: 0,
...
test("throwing tail time accessors fall back before committing partial index state", () => {
  const projector: CallingStatefulProjector = {
    id: "throwing-time-accessor-test",
    calls: 0,
    oneToOne: false,
    supportsStatefulTailProjection: true,
    project(rows = [], options = {}) {
...
for (const operation of ["append", "replace"]) {
  test(`stateful ${operation} reads only a constant-sized source/checkpoint tail`, () => {
    ...
    const drawingLineageIndex = mustBeDefined(store.drawingLineageIndex());
  ...
  test(`${descriptor.name} repeated append and replace-last stays full-projection equivalent`, () => {
    ...
    for (let operation = 0; operation < 160; operation += 1) {
      ...
      for (const previousRow of previousDisplay) {
        if (isOrdinalAxisTime(previousRow.time) && !retainedOrders.has(previousRow.time.order)) {
...
test("semantic metadata changes are not hidden by an unchanged synthetic order and OHLC", () => {
  ...
  const insertedCustomValues = mustBeDefined(mustBeDefined(patch.insert[0]).customValues);
  const nested = insertedCustomValues.nested;
  assert.ok(nested && typeof nested === "object" && "revision" in nested);
  assert.equal(nested.revision, "new");
});

test("cyclic metadata with different graph topology is not reused as a common prefix", () => {
  const oldGraph: GraphNode = { label: "head" };
  oldGraph.next = oldGraph;
  const newGraph: GraphNode = { label: "head" };
  const newTail: GraphNode = { label: "head" };
  ...
  assert.equal(patch.deleteCount, 1);
  assert.equal(patch.insert.length, 1);
  const graphValues: ProjectionCustomValues = mustBeDefined(
    mustBeDefined(patch.insert[0]).customValues,
  );
  const graph = graphValues.graph;
  assert.ok(graph && typeof graph === "object" && "label" in graph && "next" in graph);
  const graphNext = graph.next;
  assert.ok(graphNext && typeof graphNext === "object" && "next" in graphNext);
  assert.notStrictEqual(graphNext, graph);
  assert.strictEqual(graphNext.next, graphNext);
});
