...
test("visible ranges write the composite identity key", () => {
  withLocalStorage({}, (storage) => {
    ...
    const saved: Record<string, { barSpacing: number; rightOffset: number; rightmostTime: number; savedAt: number }> = JSON.parse(
      mustBeDefined(storage.getItem(VISIBLE_RANGE_KEY)),
    );
    assert.deepEqual(
      {
