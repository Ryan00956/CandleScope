...
function mockPrimitive(id: string) {
  const calls: boolean[] = [];
  return {
    id,
    calls,
...
type HoverHit = NonNullable<Parameters<typeof cursorForLineToolHit>[0]>;

function partialHit(value: object): HoverHit {
  return malformedFixture<HoverHit>(value);
}
...
test("cursor helpers map drawing hit zones to expected cursors", () => {
  ...
  assert.equal(cursorForLineToolHit(partialHit({ type: "line", pointIndex: 0 })), "crosshair");
  assert.equal(cursorForLineToolHit(partialHit({ type: "line", pointIndex: -1 })), "move");
  assert.equal(cursorForLineToolHit(null), "crosshair");

  assert.equal(cursorForShapeToolHit(partialHit({ type: "shape", zone: "l" })), "ew-resize");
  assert.equal(cursorForShapeToolHit(partialHit({ type: "shape", zone: "tl" })), "nwse-resize");
  assert.equal(cursorForShapeToolHit(partialHit({ type: "shape", zone: "body" })), "move");
  ...
  assert.equal(cursorForTextToolHit(partialHit({ type: "text", handle: "r" })), "ew-resize");
  assert.equal(cursorForTextToolHit(partialHit({ type: "text", handle: "br" })), "nwse-resize");
  assert.equal(cursorForTextToolHit(partialHit({ type: "text" })), "move");
