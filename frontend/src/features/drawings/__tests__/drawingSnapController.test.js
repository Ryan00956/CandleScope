...
test("invalid ordinal anchors are never copied into drawing time", () => {
  const { adapter } = ordinalAdapter();
  adapter.axisTimeToDrawingAnchor = () => ({
    time: 1_700_000_000,
    sourceOrdinal: -1,
    sourceProjection: "renko",
  });
  ...
  assert.equal(typeof snapped.time, "number");
  assert.notEqual(typeof snapped.time, "object");
  assert.equal(snapped.price, 100);
});
