...
import type {
  DrawingExportLifecycleSnapshot,
} from "../export/drawingExportLifecycle.js";
...
export interface DrawingPerfDebugHandle {
  ...
  readonly readExportLifecycle: () => DrawingExportLifecycleSnapshot;
...
function createDurationHistograms(
  ...
  return {
    ...
    persistenceChunkMs: new RollingDurationHistogram(capacity),
    restoreChunkMs: new RollingDurationHistogram(capacity),
...
function createRawCaptures(
  ...
  return {
    ...
    persistenceChunkMs: new BoundedRawCapture(capacity),
    restoreChunkMs: new BoundedRawCapture(capacity),
...
function emptyRawCaptureSnapshot(): DrawingPerfRawCaptureSnapshot {
  return {
    ...
    metrics: {
      ...
      persistenceChunkMs: emptyRawMetricCapture(),
      restoreChunkMs: emptyRawMetricCapture(),
  ...
  private rawCaptureSnapshot(drain: boolean): DrawingPerfRawCaptureSnapshot {
    ...
    return {
      ...
      metrics: {
        ...
        persistenceChunkMs: read(this.rawCaptures.persistenceChunkMs),
        restoreChunkMs: read(this.rawCaptures.restoreChunkMs),
  ...
  private snapshotAt(capturedAtMs: number): DrawingPerfSnapshot {
    return {
      ...
      durations: {
        ...
        persistenceChunkMs: this.histograms.persistenceChunkMs.snapshot(),
        restoreChunkMs: this.histograms.restoreChunkMs.snapshot(),
...
export function installDrawingPerfDebugHandle(
  ...
  const handle: DrawingPerfDebugHandle = Object.freeze({
    ...
    readExportLifecycle: () => readDrawingExportLifecycle(),
