...
test("watchlist full cache stores and reads rows by symbolKey and interval", () => {
  ...
  mergeFullCacheRows(
    ...
    [
      ...
      { time: 1060, open: 2, high: 3, low: 2, close: 3, volume: 11 },
    ],
    { status: "warm", source: "latest" },
  );

  const entry = getFullCacheEntry("okx:spot:BTC-USDT", "1h");
  assert.equal(entry.status, "warm");
  assert.deepEqual(entry.coverage, { firstTime: 1000, lastTime: 1060, bars: 2 });

  const warm = getWarmRows("okx:spot:BTC-USDT", "1h");
...
test("watchlist full cache patches realtime klines without replacing other intervals", () => {
  ...
  mergeFullCacheRows("spot:BTCUSDT", "1m", [{ time: 1000, close: 1 }]);
  mergeFullCacheRows("spot:BTCUSDT", "1h", [{ time: 1000, close: 10 }]);

  patchFullCacheRealtimeKline("spot:BTCUSDT", "1m", { time: 1060, close: 2 });
  patchFullCacheRealtimeKline("spot:BTCUSDT", "1h", { time: 1000, close: 11 });

  assert.deepEqual(
    getWarmRows("spot:BTCUSDT", "1m").rows.map((row) => row.close),
    [1, 2],
  );
  assert.deepEqual(
    getWarmRows("spot:BTCUSDT", "1h").rows.map((row) => row.close),
    [11],
  );
});
