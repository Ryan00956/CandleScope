...
test("watchlist full cache keeps a concurrent forming WS update over an older forming REST row", () => {
  resetWatchlistFullCache();
  ...
  assert.deepEqual(merged.rows, [{ time, close: 12, is_closed: false }]);
  assert.equal(merged.source, "ws");
  assert.equal(merged.lastRealtimeMs, 200);
});
