...
function mergeByTime(
  current: KlineBar[],
  incoming: KlineBar[],
  preserveCurrent?: (current: KlineBar, incoming: KlineBar) => boolean,
): KlineBar[] {
  ...
  for (const item of incoming) {
    const existing = uniq.get(item.time);
    if (existing && preserveCurrent?.(existing, item)) continue;
    uniq.set(item.time, item);
  }
    ...
    ? mergeByTime(
      ...
      expectedRealtimeVersion == null
        ...
        : (currentRow, incomingRow) => (
          realtimeRowChangedAfter(
            symbolKey,
            interval,
            incomingRow.time,
            expectedRealtimeVersion,
          )
          // A newly final HTTP row may close a concurrently forming tail.
          // Otherwise a response that started before a realtime mutation is
          // stale for that timestamp, including forming-bar mutations.
          && !(currentRow.is_closed !== true && incomingRow.is_closed === true)
        ),
    )
