...
function stripRuntimeFields(indicator) {
...
export function useActiveIndicators({ onRequireCompute } = {}) {
  const [activeIndicators, setActiveIndicators] = useState(loadActiveIndicators);
  const volInitRef = useRef(false);

  useEffect(() => {
    saveActiveIndicators(activeIndicators.map(stripRuntimeFields));
  }, [activeIndicators]);
  ...
  useEffect(() => {
    ...
    // Always try to fetch and add vol. Older versions may have set the
    // initialization flag despite the fetch failing.
    fetchPreset("vol")
      .then((full) => {
        if (!full) return;
