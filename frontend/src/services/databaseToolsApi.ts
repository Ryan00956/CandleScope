...
export type DatabaseSeriesStatus = "healthy" | "gap" | "stale";
...
export interface DatabaseSeriesFilter {
    exchange?: string;
    marketType?: string;
}

export interface DatabaseSeriesListResult {
    mode: "mock";
...
export interface DatabaseActionResult extends Record<string, unknown> {
    status: string;
    message: string;
}

const MOCK_SERIES: DatabaseSeries[] = [
    {
        exchange: "binance",
        marketType: "spot",
        symbol: "BTCUSDT",
...
function wait(ms = 240): Promise<void> {
    return new Promise<void>((resolve) => {
        setTimeout(resolve, ms);
    });
}

function cloneSeries(series: DatabaseSeries[]): DatabaseSeries[] {
    return series.map(item => ({ ...item }));
}

export async function fetchDatabaseSeries({
    exchange = "",
    marketType = "",
}: DatabaseSeriesFilter = {}): Promise<DatabaseSeriesListResult> {
    await wait();
...
export async function scanDatabaseSeriesGaps(
    seriesKey: Partial<DatabaseSeries> | null | undefined,
...
export async function requestDatabaseBackfill({
    seriesKey,
    targetEndMs,
}: {
    seriesKey: Partial<DatabaseSeries> | null | undefined;
    targetEndMs: number | null | undefined;
}): Promise<DatabaseActionResult> {
    await wait(420);
...
export async function deleteDatabaseSeries(
    seriesKey: Partial<DatabaseSeries> | null | undefined,
): Promise<DatabaseActionResult> {
    await wait(360);
    return {
...
export async function deleteDatabaseSymbol({
    symbol,
    intervals = [],
}: {
    symbol: string;
    intervals?: Array<Partial<DatabaseSeries>>;
}): Promise<DatabaseActionResult> {
    await wait(420);
