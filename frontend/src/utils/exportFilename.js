...
export function getExportExtension(format = "png") {
  return EXPORT_EXTENSION_BY_FORMAT[format] || EXPORT_EXTENSION_BY_FORMAT.png;
}

export function getExportMimeType(format = "png") {
  return EXPORT_MIME_BY_FORMAT[format] || EXPORT_MIME_BY_FORMAT.png;
}
...
export function buildDefaultWatermark({ exchange, marketType, symbol, interval } = {}) {
...
export function estimateExportPixels(width, height, scale = 1) {
...
export function assertExportPixelBudget(width, height, scale = 1, maxPixels = DEFAULT_MAX_EXPORT_PIXELS) {
