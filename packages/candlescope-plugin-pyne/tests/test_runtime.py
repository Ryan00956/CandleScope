...
def test_descriptor_is_public_versioned_and_reports_the_engine_boundary() -> None:
    ...
    assert descriptor.version == PLUGIN_VERSION == "0.1.0"
    ...
    assert descriptor.meta["renderCoverage"] == ["lines"]
...
def test_descriptor_rejects_an_unpinned_installed_engine_version(
    ...
    with pytest.raises(RuntimeError, match="requires pyne-runtime 0.2.0rc1"):
...
def test_execute_maps_line_series_and_reports_untransported_output_kinds() -> None:
    ...
    assert result.output.meta == {
        ...
        "unsupportedOutputKinds": ["hlines", "markers"],
...
def test_series_ids_are_normalized_and_deduplicated(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        runtime_module.pyne_runtime,
        "execute_pyne_script",
        lambda **_: SimpleNamespace(ok=True, lines=lines, output={}, meta={}),
    )
